...
void* create_node(void* node){
	if(!node) pthread_exit(NULL);
	struct Node* ptr = generate_data_node();
	if(!ptr) pthread_exit(NULL);
	pthread_mutex_lock(&mutex_lock);
	ptr->data = 1;
	if(List->header == NULL) List->header = List->tail = ptr;
	else{
		List->tail->next = ptr;
		List->tail = ptr;
	}
...
void * producer_thread( void *arg)
{
    ...
    struct Node * ptr;
    struct list partialList;
    partialList.header = partialList.tail = generate_data_node();
    int counter = 0;  
    /* generate and attach K nodes to the global list */

    for(; counter < K && counter < K; counter++){
		ptr = generate_data_node();
		ptr->data = 1;
		partialList.tail->next = ptr;
		partialList.tail = ptr;
    }
    pthread_mutex_lock(&mutex_lock);
    if(List->header == NULL){
		List->header = partialList.header;
		List->tail = partialList.tail;
    }
    else{
		List->tail->next = partialList.header;
		List->tail = partialList.tail;
    }
    pthread_mutex_unlock(&mutex_lock);
}
